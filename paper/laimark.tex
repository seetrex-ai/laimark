...
% ==========================================================
\begin{abstract}
Reinforcement learning from verifiable reward (RLVR), as used in \textsc{DeepSeek-R1} and related systems, relies on curated external benchmarks of problems paired with automatic evaluators.
We ask whether the same model can generate its own calibrated curriculum and recover the gains of external benchmarks via GRPO.
On HumanEval with Qwen3-8B, a self-generation pipeline produces candidate programming problems with reference solutions and unit tests, and retains only those in the learnability window (solved some but not all of the time by the base model).
GRPO trained on tens of self-generated problems reaches 76.8\% pass@1 (vs.\ 63.4\% base), capturing roughly 65\% of the gain obtained by the same method on hundreds of curated problems from HumanEval and MBPP (84.1\%).
We then document three structural limits that cap this approach as a route to continued self-improvement.
\textbf{(1) Iteration does not accumulate}: a second GRPO round on problems calibrated against the first-round checkpoint fails to improve over it---the checkpoint dominates the problems it helped select.
\textbf{(2) Task-type imbalance destroys transfer}: a curriculum with $\sim$84\% abduction-style problems drops pass@1 to 61.0\% on inductive HumanEval, below the pre-training baseline.
\textbf{(3) Self-generation becomes inapplicable at scale}: with Qwen3-32B (89.0\% pass@1 without training), less than 1\% of generated medium problems and 0\% of hard problems pass calibration---a generator-solver pair capable of producing well-formed verified problems is also capable of solving them.
A per-problem analysis further shows that approximately 80\% of GRPO's improvement on calibrated problems is formatting (code indentation) rather than new reasoning capability.
These results position self-generated curricula as a regularizer effective when the base model has substantial room to grow, not a route to closed-loop improvement at scale.
\end{abstract}
...
Reinforcement learning from verifiable reward (RLVR) has become a dominant post-training technique for language models with reasoning requirements.
\textsc{DeepSeek-R1}~\citep{deepseek2025r1} demonstrated that on problems with automatic evaluators---mathematical proofs, unit-tested programs, multiple-choice answers---reinforcement learning with Group Relative Policy Optimization (GRPO)~\citep{shao2024grpo} can elicit substantial reasoning capability from a pretrained base model without human preference data.
Subsequent work has shown that the same technique, applied at scale, improves code generation, mathematical reasoning, and formal verification benchmarks by orders of magnitude more than the pretraining signal alone would suggest.

Every such system so far depends on a \emph{curated external curriculum}: benchmarks of problems, assembled and verified by humans, that serve as the training distribution.
HumanEval~\citep{chen2021codex}, MBPP~\citep{austin2021mbpp}, and their successors provide the programming problems; GSM8K~\citep{cobbe2021training} and MATH provide the mathematics.
The automatic evaluators (unit tests, answer checkers) are cheap to run, but the problems themselves must be written by someone.
This dependency has practical cost and a conceptual one: the model cannot improve on problems no one thought to write down.

An alternative is self-generation.
Absolute Zero Reasoner (AZR)~\citep{zhao2025azr} trains proposer and solver networks on self-generated problems with verifiable answers, reporting reasoning gains without curated training data.
The Darwin G{\"o}del Machine (DGM)~\citep{zhang2025dgm} frames self-improvement at the agent level: a language-model agent modifies its own code and retains variants that score higher on a held-out task distribution.
Both lines raise a natural question: \emph{how far can self-generation go?}
If a model can generate its own problems and verify them, can it train on them and match the performance of externally curated benchmarks---or beyond?

We study this question at a deliberately modest scale where the mechanism can be instrumented directly (Qwen3-8B~\citep{qwen3} as base model, HumanEval as the held-out evaluation).
Our starting point is that RLVR with GRPO on curated benchmarks works: on Qwen3-8B with hundreds of problems from HumanEval and MBPP, GRPO improves pass@1 from 63.4\% to 84.1\% (measured with the official HuggingFace fp16 evaluator, +34 problems solved, 0 regressions).
The question is whether the same base model, given only its own generations and a Python interpreter as evaluator, can recover this gain.

\paragraph{Findings, positive.}
Partially, yes.
A self-generation pipeline with calibration---retaining only problems the base model solves sometimes but not always---produces meaningful gains via GRPO: 63.4\% $\to$ 76.8\% pass@1 on HumanEval, using tens of generated problems and no external benchmarks.
This captures roughly 65\% of the improvement obtained with hundreds of curated problems.
A methodological refinement---doubling the within-batch contrast by increasing GRPO's \texttt{num\_generations}---provides most of the remaining gain at fixed data volume, suggesting that gradient-signal contrast dominates data volume for small curricula.

\paragraph{Findings, negative.}
The approach does not scale, in three distinct ways, each with a different mechanism.
\textbf{(i) Iteration does not accumulate.}
Once a checkpoint has been trained on its own calibrated curriculum, a second round trained on problems calibrated against that checkpoint fails to improve over it.
The checkpoint dominates the problems it helped select.
\textbf{(ii) Task-type imbalance destroys transfer.}
When adversarial co-evolution of the curriculum shifts the task-type distribution---e.g., a pool with $\sim$84\% abduction-style problems---performance on the target inductive benchmark drops to 61.0\% pass@1, below the pre-training baseline of the calibration method.
Diversity in the curriculum acts as a regularizer; diversity concentrated in a single incompatible type is actively destructive.
\textbf{(iii) Self-generation becomes inapplicable at scale.}
Moving from Qwen3-8B (63.4\% base) to Qwen3-32B (89.0\% base) collapses the learnability window.
Less than 1\% of problems the 32B model generates are solved sometimes-but-not-always by itself; 0\% of problems it labels as ``hard'' pass calibration.
A generator-solver pair capable of producing well-formed problems with verified solutions is also capable of solving them.

\paragraph{What GRPO teaches, qualitatively.}
A per-problem analysis of the trained checkpoint reveals that approximately 80\% of GRPO's improvement on calibrated problems is formatting---specifically, emitting correctly indented Python---rather than new reasoning capability.
The remaining 20\% includes at least one case of genuine logical improvement (distinguishing datetime format strings from regex patterns).
The common framing of RLVR as ``teaching the model to reason'' overstates the mechanism at this scale; ``teaching the model to present its reasoning in the evaluator's expected format'' is a more accurate description of most of what happens.
...
\begin{enumerate}
    \item We demonstrate that GRPO on self-generated calibrated problems recovers roughly 65\% of the gain of externally curated benchmarks on HumanEval with Qwen3-8B, with zero external problems (\S\ref{sec:main}).
    \item We show that within-batch contrast in GRPO dominates data volume on small curricula: doubling \texttt{num\_generations} captures most of the remaining gain without additional data (\S\ref{sec:main}).
    \item We document three structural limits of closed-loop self-evolution---iteration non-accumulation, task-type interference, and inapplicability at scale---each with its own mechanism and direct empirical evidence (\S\ref{sec:limits}).
    \item We decompose GRPO's per-problem improvement into formatting ($\sim$80\%) and reasoning capability ($\sim$20\%), qualifying a common claim about what RLVR teaches (\S\ref{sec:formatting}).
    \item We position LAIMARK as a closed-loop self-evolution system in which prompt evolution, weight update, self-refinement, and self-generated curricula run on a single base model, and characterize its differentiators from DGM / Hyperagents, DeepSeek-R1, and AZR along this four-component axis (\S\ref{sec:background-l2}, Fig.~\ref{fig:loop}).
\end{enumerate}

\paragraph{Scope.}
This paper reports LAIMARK's components 2 and 4 in detail and components 1 and 3 as context necessary for interpreting the results.
We do not study agent-level self-modification of the training code or reward function---the G{\"o}del-Machine~\citep{schmidhuber2003godel} direction or the Hyperagents~\citep{zhang2026hyperagents} direction---nor signal sources outside the model's distribution (external perception, physical environments, human feedback).
Both are orthogonal extensions that the structural limits we document motivate directly; we discuss them in \S\ref{sec:discussion}.
All experimental results reported use the official HuggingFace fp16 HumanEval evaluator.
...
\node[promptop] (prompt)  at (-4.3,  1.9)
    {\textbf{1. Prompt evolution}\\[2pt]
     {\glyph $\mathtt{txt}\!\to\!\mathtt{txt}$}~\scriptsize evolve system prompt\\[-1pt]
     \scriptsize weights unchanged};

\node[weightop] (weights) at ( 4.3,  1.9)
    {\textbf{2. Weight update}\\[2pt]
     {\glyph $\nabla\!_\theta$}~\scriptsize GRPO on\\[-1pt]
     \scriptsize verifiable reward};

\node[weightop] (selfgen) at ( 4.3, -1.9)
    {\textbf{4. Self-generated curriculum}\\[2pt]
     {\glyph $\pi\!\to\!(x,\text{tests},\hat{y})$}~\scriptsize problems,\\[-1pt]
     \scriptsize tests, reference sol.};

\node[promptop] (refine)  at (-4.3, -1.9)
    {\textbf{3. Self-refinement}\\[2pt]
     {\glyph $\mathtt{txt}\!\to\!\mathtt{txt}$}~\scriptsize re-enter comp.\,1\\[-1pt]
...
Note that $A_k = 0$ for every rollout whenever all $r_k$ are equal.
A prompt on which the policy always passes---or always fails---contributes nothing to training.
...
One observation from the prompt-evolution phase matters for what follows.
L2b does not add reasoning capability; it adds an \emph{output-distribution shift} that biases the model toward procedural, step-by-step code.
Its effect depends on the model having procedural code underrepresented in its default output distribution.
Section~\ref{sec:limits-scale} shows that Qwen3-32B, whose default distribution is already more procedural, loses 1.2 pass@1 points under L2b: the same prompt that improves 8B degrades 32B.
This is the first datum in a pattern we document throughout: closed-loop RLVR at this scale produces output-distribution shaping, not capability acquisition (\S\ref{sec:formatting}).
...
Empirically on v1 vs.\ v2, the fraction of training steps where the group was non-degenerate rose from approximately 30\% to approximately 80\%.
...
The mechanism is direct: calibration requires $\hat{p}(x) \in [0.2, 0.8]$, and 32B's $\hat{p}(x)$ is approximately 1 on essentially every problem it formulates with a verified reference solution.
...
The run produced a refined system prompt that improved the checkpoint's pass@1 by approximately two points on a deployment-quantized evaluation setup that is not directly comparable to the HF fp16 harness used for the headline results of this paper.
